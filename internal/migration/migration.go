...
// Migrate checks for and applies any pending database migrations.
func Migrate(db *gorm.DB) error {
	slog.Info("Checking for pending database migrations")

	if err := migrateDeviceType(db); err != nil {
		return fmt.Errorf("device type migration failed: %w", err)
	}
...
func migrateDeviceType(db *gorm.DB) error {
	// Only run migration if the 'devices' table exists
	if !db.Migrator().HasTable(&data.Device{}) {
		slog.Info("Skipping DeviceType migration: 'devices' table not found.")
		return nil
	}

	columnTypes, err := db.Migrator().ColumnTypes(&data.Device{})
	if err != nil {
		return fmt.Errorf("could not get column types for device: %w", err)
	}
	...
	for _, ct := range columnTypes {
		if ct.Name() == "type" {
			...
			// This check is a bit broad, but should catch VARCHAR, TEXT, etc.
			// Integers would be INT, INTEGER, etc.
			if strings.Contains(strings.ToLower(dbType), "char") || strings.Contains(strings.ToLower(dbType), "text") {
				needsMigration = true
				break
			}
		}
	...
	if !needsMigration {
		slog.Info("DeviceType migration not needed.")
		return nil
	}

	slog.Info("Starting DeviceType migration from string to int.")

	// Use a transaction
	return db.Transaction(func(tx *gorm.DB) error {
		// 1. Rename old column
		if err := tx.Migrator().RenameColumn(&data.Device{}, "type", "type_old"); err != nil {
			return fmt.Errorf("failed to rename 'type' column: %w", err)
		}
		slog.Info("Renamed 'type' column to 'type_old'")

		// 2. AutoMigrate to create new schema (including the new 'type' int column)
		// This must be done outside the transaction for some DBs, but GORM handles it.
		if err := tx.AutoMigrate(&data.Device{}); err != nil {
			return fmt.Errorf("failed to auto-migrate Device schema: %w", err)
		}
		...
		updateSQL := fmt.Sprintf("UPDATE devices SET type = %s", caseBuilder.String())
		if err := tx.Exec(updateSQL).Error; err != nil {
			return fmt.Errorf("failed to update 'type' column data: %w", err)
		}
		slog.Info("Successfully migrated data from 'type_old' to 'type'")

		// 4. Drop old column
		if err := tx.Migrator().DropColumn(&data.Device{}, "type_old"); err != nil {
			return fmt.Errorf("failed to drop 'type_old' column: %w", err)
		}
		slog.Info("Dropped 'type_old' column")

		slog.Info("DeviceType migration completed successfully.")
		return nil
	})
}
