...
*   **Database:** SQLite with **GORM** (ORM). Data is stored in a relational schema (Users, Devices, Apps).
*   **Templating:** `html/template` (standard library).
*   **Rendering:** Native Go integration with `github.com/tronbyt/pixlet`.
*   **Authentication:** Session-based (secure cookies via `gorilla/sessions`) with Scrypt password hashing.
...
2.  **Run:**
    ```bash
    docker run -p 8080:8080 -v $(pwd)/data:/app/data tronbyt-server
    ```
...
*   [x] **Core API:** `/v0/devices/{id}`, `/v0/devices/{id}/next` (Rotation logic).
*   [x] **Auth:** Login/Logout, Password Check (Scrypt), Session Cookies.
*   [x] **Web UI:** Login Page, Dashboard (Device Cards), i18n (German), Create Device Form.
*   [x] **Reverse Proxy:** Support for `X-Forwarded-Proto` and `X-Forwarded-Host` headers (via `ProxyMiddleware`).
