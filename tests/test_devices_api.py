...
class TestDevicesEndpoint:
    def test_list_devices_success(self, auth_client: TestClient, device_user: User) -> None:
        """Test successful listing of devices"""
        api_key = device_user.api_key
        response = auth_client.get(
            "/api/v0/devices", headers={"Authorization": f"Bearer {api_key}"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_list_devices_with_direct_auth_header(
        ...
        api_key = device_user.api_key
        response = auth_client.get("/api/v0/devices", headers={"Authorization": api_key})
        assert response.status_code == 200
        data = response.json()
        assert "devices" in data
        assert len(data["devices"]) == 1
    ...
    def test_list_devices_missing_auth_header(self, client: TestClient) -> None:
        ...
        response = client.get("/api/v0/devices")
    ...
    def test_list_devices_invalid_api_key(self, auth_client: TestClient) -> None:
        ...
        response = auth_client.get(
            "/api/v0/devices", headers={"Authorization": "Bearer invalid_key"}
        )
        assert response.status_code == 401
        assert "Invalid credentials" in response.text
...
class TestDeviceEndpoint:
    ...
    def test_get_device_not_found(self, auth_client: TestClient, device_user: User) -> None:
        """Test device retrieval for non-existent device"""
        api_key = device_user.api_key
        response = auth_client.get(
            "/api/v0/devices/nonexistent",
            headers={"Authorization": f"Bearer {api_key}"},
        )
    ...
    def test_patch_device_missing_auth(
        ...
        device_id = list(device_user.devices.keys())[0]
        response = auth_client.patch(
            f"/api/v0/devices/{device_id}", json={"brightness": 100}
        )
        assert response.status_code == 400
