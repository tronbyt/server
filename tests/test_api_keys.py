...
class TestApiKeyGeneration:
    ...
    def test_migrate_user_api_keys_generates_key_for_user_without_key(
        ...
        db.save_user(conn, user.model_dump())
        conn.close()

        conn = utils._get_db_conn()
        user_without_key_data = db.get_user(conn, "testuser")
        assert user_without_key_data is not None
        user_without_key = User(**user_without_key_data)
        ...
        conn = utils._get_db_conn()
        migrated_user_data = db.get_user(conn, "testuser")
        assert migrated_user_data is not None
        migrated_user = User(**migrated_user_data)
    ...
    def test_migrate_user_api_keys_preserves_existing_key(
        ...
        conn = utils._get_db_conn()
        user_after_migration_data = db.get_user(conn, "testuser")
        assert user_after_migration_data is not None
        user_after_migration = User(**user_after_migration_data)
    ...
    def test_migrate_user_api_keys_handles_multiple_users(
        ...
        conn = utils._get_db_conn()
        user1_data = utils.db.get_user(conn, "user1")
        assert user1_data is not None
        user1 = User(**user1_data)
        user1.api_key = ""
        utils.db.save_user(conn, user1.model_dump())
        conn.close()

        conn = utils._get_db_conn()
        user2_data = utils.db.get_user(conn, "user2")
        ...
        conn = utils._get_db_conn()
        utils.db.migrate_user_api_keys(conn)
        conn.close()

        conn = utils._get_db_conn()
        user1_after_data = utils.db.get_user(conn, "user1")
        assert user1_after_data is not None
        user1_after = User(**user1_after_data)
        assert user1_after.api_key
        assert len(user1_after.api_key) == 32
        conn.close()

        conn = utils._get_db_conn()
        user2_after_data = utils.db.get_user(conn, "user2")
        assert user2_after_data is not None
        user2_after = User(**user2_after_data)
...
class TestApiKeyRetrieval:
    ...
    def test_get_user_by_api_key_success(self, auth_client: TestClient) -> None:
        ...
        conn = utils._get_db_conn()
        retrieved_user_data = db.get_user_by_api_key(conn, api_key)
        assert retrieved_user_data is not None
    ...
    def test_get_user_by_api_key_not_found(self, auth_client: TestClient) -> None:
        ...
        conn = utils._get_db_conn()
        retrieved_user = utils.db.get_user_by_api_key(conn, "nonexistent_key")
    ...
    def test_get_user_by_api_key_empty_string(self, auth_client: TestClient) -> None:
        ...
        conn = utils._get_db_conn()
        retrieved_user = utils.db.get_user_by_api_key(conn, "")
        assert retrieved_user is None
        conn.close()
    ...
    def test_get_user_by_api_key_multiple_users(self, auth_client: TestClient) -> None:
        ...
        user1_data = utils.db.get_user(conn, "user1")
        user2_data = utils.db.get_user(conn, "user2")
        assert user1_data is not None and user2_data is not None
        user1 = User(**user1_data)
        user2 = User(**user2_data)
        conn.close()
        ...
        conn = utils._get_db_conn()
        retrieved_user1_data = utils.db.get_user_by_api_key(conn, api_key1)
        assert retrieved_user1_data is not None
        retrieved_user1 = User(**retrieved_user1_data)
        assert retrieved_user1.username == "user1"
        conn.close()

        conn = utils._get_db_conn()
        retrieved_user2_data = utils.db.get_user_by_api_key(conn, api_key2)
        assert retrieved_user2_data is not None
        retrieved_user2 = User(**retrieved_user2_data)
    ...
    def test_get_user_by_api_key_case_sensitive(self, auth_client: TestClient) -> None:
        ...
        conn = utils._get_db_conn()
        retrieved_user = utils.db.get_user_by_api_key(conn, wrong_case_key)
        assert retrieved_user is None
        conn.close()

        conn = utils._get_db_conn()
        retrieved_user = utils.db.get_user_by_api_key(conn, api_key)
...
class TestApiKeyIntegration:
    ...
    def test_user_registration_creates_api_key(self, auth_client: TestClient) -> None:
        ...
        conn = utils._get_db_conn()
        user_data = utils.db.get_user(conn, "newuser")
...
class TestApiKeyIntegration:
    def test_api_key_authentication_flow(self, auth_client: TestClient) -> None:
        ...
        response = auth_client.get(f"/api/v0/devices/{device_id}")
    ...
    def test_api_key_uniqueness(self, auth_client: TestClient) -> None:
        ...
        for i in range(5):
            ...
            conn = utils._get_db_conn()
            user_data = utils.db.get_user(conn, username)
            assert user_data is not None
            user = User(**user_data)
