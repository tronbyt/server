...
def test_api(registered_client: TestClient) -> None:
    ...
    # call next
    registered_client.get(f"/{device_id}/next")
    ...
    assert not Path(f"users/webp/{device_id}").is_dir()


class TestMoveApp:
    def _setup_device_with_apps(
        self, registered_client: TestClient, num_apps: int = 4
    ...
    def _get_sorted_apps(self, device_id: str) -> list[App]:
        ...
        return [App(**app) for app in apps_list]

    def test_move_app_scenarios(self, registered_client: TestClient) -> None:
        device_id, _ = self._setup_device_with_apps(registered_client, 4)
        ...
        registered_client.get(f"/{device_id}/app2/moveapp?direction=down")
        ...
        registered_client.get(f"/{device_id}/app2/moveapp?direction=up")
        ...
        registered_client.get(f"/{device_id}/app1/moveapp?direction=up")
        apps = self._get_sorted_apps(device_id)
        assert apps[0].iname == "app1" and apps[0].order == 0
        ...
        registered_client.get(f"/{device_id}/app4/moveapp?direction=down")
        apps = self._get_sorted_apps(device_id)
        assert apps[0].iname == "app1" and apps[0].order == 0
        assert apps[1].iname == "app2" and apps[1].order == 1
        assert apps[2].iname == "app3" and apps[2].order == 2
        ...
        if user and "devices" in user and device_id in user["devices"]:
            ...
            db.save_user(logger, user)
        db.delete_user(logger, "testuser")  # If utils.register_user doesn't clean up
