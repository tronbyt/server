...
def _next_app_logic(
    ...
    user_data = db.get_user_by_device_id(db_conn, device_id)
    if not user_data:
    ...
    print(f"Checking for pushed images in {pushed_dir}")
    ...
    if (
        ...
        and device.night_mode_app in device.apps
    ):
...
def index(
    ...
    if user.devices:
        ...
        if user_updated:
            db.save_user(db_conn, user.model_dump())
...
def create(request: Request, user: User = Depends(manager)) -> Response:
    ...
    return templates.TemplateResponse(
        request, "manager/create.html", {"user": user}
    )
...
def create_post(
    ...
    error = None
    if not name or db.get_device_by_name(user.model_dump(), name):
        error = "Unique name is required."
    if error is not None:
        flash(request, error)
        return templates.TemplateResponse(
            request, "manager/create.html", {"user": user}, status_code=status.HTTP_400_BAD_REQUEST
        )
    ...
    for _ in range(max_attempts):
        ...
        return templates.TemplateResponse(
            request, "manager/create.html", {"user": user}, status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
    ...
    if db.save_user(
        db_conn, user.model_dump()
    ) and not db.get_device_webp_dir(device.id).is_dir():
...
def addapp_post(
    ...
    db_conn: sqlite3.Connection = Depends(get_db),
    name: str = Form(...),
    uinterval: Optional[int] = Form(None),
    display_time: Optional[int] = Form(None),
    ...
    app = App(
        ...
        else app_details.get("recommended_interval"),
...
def uploadapp(request: Request, device_id: str, user: User = Depends(manager)) -> Response:
...
def deleteupload(
    ...
    if any(
        ...
        db.delete_user_upload(user.model_dump(), filename)
...
async def configapp_post(
    ...
    if image is not None:
        ...
        db.save_app(db_conn, device_id, app.model_dump())
    else:
...
def preview(
    ...
    try:
        ...
        if data is None:
            raise HTTPException(
                status_code=500, detail="Error running pixlet render"
            )
...
def adminindex(
    ...
    users = [User(**u) for u in db.get_all_users(db_conn)]
    return templates.TemplateResponse(
...
def deleteuser(
    ...
    return RedirectResponse(
        url="/adminindex", status_code=status.HTTP_302_FOUND
    )
...
def generate_firmware_post(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Firmware generation failed"
        )
...
def set_user_repo(
    ...
    if set_repo(request, user, "app_repo_url", apps_path, app_repo_url):
        return RedirectResponse(url="/", status_code=status.HTTP_302_FOUND)
...
def refresh_user_repo(
    ...
    apps_path = db.get_users_dir() / user.username / "apps"
    if set_repo(request, user, "app_repo_url", apps_path, user.app_repo_url):
        return RedirectResponse(url="/", status_code=status.HTTP_302_FOUND)
    return RedirectResponse(url="/auth/edit", status_code=status.HTTP_302_FOUND)
...
def export_device_config(device_id: str, user: User = Depends(manager)) -> Response:
    ...
    return Response(
        ...
        media_type="application/json",
        headers={
            "Content-Disposition": f"attachment;filename={device_id}_config.json"
        },
    )
...
async def import_user_config(
    ...
    if not file.filename:
        flash(request, "No selected file")
        return RedirectResponse(
            url="/auth/edit", status_code=status.HTTP_302_FOUND
        )
    if not file.filename.endswith(".json"):
        flash(request, "Invalid file type. Please upload a JSON file.")
        return RedirectResponse(
    ...
    try:
        ...
        db.save_user(db_conn, new_user.model_dump())
...
def get_brightness(device_id: str, db_conn: sqlite3.Connection = Depends(get_db)) -> Response:
...
def currentwebp(device_id: str, db_conn: sqlite3.Connection = Depends(get_db)) -> Response:
    ...
    return appwebp(db_conn, device_id, current_app_iname)


@router.get("/{device_id}/{iname}/appwebp")
def appwebp(device_id: str, iname: str, db_conn: sqlite3.Connection = Depends(get_db)):
...
async def schema_handler(
    ...
    if not device:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    app_data = device.apps.get(iname)
    if not app_data:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)
    app = App(**app_data)
