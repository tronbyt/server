...
def ws_root() -> str:
    ...
    port = settings.SERVER_PORT
    url = f"{protocol}://{hostname}"
    if (protocol == "wss" and port != "443") or (
        protocol == "ws" and port != "80"
    ):
        url += f":{port}"
    return url
...
def set_repo(
    request: Request, user: User, repo_name: str, apps_path: Path, repo_url: str
) -> bool:
...
def possibly_render(
    db_conn: sqlite3.Connection, user: User, device_id: str, app: App, logger: logging.Logger
) -> bool:
