...
def update_system_repo(base_path: Path, logger: logging.Logger) -> None:
    """Update the system apps repository and generate the apps.json list."""
        ...
        try:
            ...
            if yaml_path.exists():
                with yaml_path.open("r") as f:
                    ...
                    app_dict = app_dict.copy(update={k: v for k, v in yaml_dict.items() if hasattr(app_dict, k)})
            else:
    ...
    with (base_path / "system-apps.json").open("w") as f:
        json.dump([a.dict() for a in apps_array], f, indent=4)
