...
def initialize_pixlet_library() -> None:
    ...
    with _pixlet_lock:
        ...
        if settings.redis_url and pixlet_init_redis_cache:
            logger.info(f"Using Redis cache at {settings.redis_url}")
            pixlet_init_redis_cache(settings.redis_url.encode("utf-8"))
        elif pixlet_init_cache:
            pixlet_init_cache()
        _pixlet_initialized = True


def c_char_p_to_string(c_pointer: ctypes.c_void_p) -> Optional[str]:
    if not c_pointer:
        return None
    data = ctypes.string_at(c_pointer)
    result = data.decode("utf-8")
    if pixlet_free_bytes:
        pixlet_free_bytes(c_pointer)
    return result
