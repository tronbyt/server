...
class RedisWaiter(Waiter):
    ...
    def __init__(self, redis_client: redis.Redis, device_id: str):
        ...
        self._pubsub = redis_client.pubsub(ignore_subscribe_messages=True)  # type: ignore[no-untyped-call,no-untyped-def]
        self._device_id = device_id
...
class RedisSyncManager(SyncManager):
    ...
    def __init__(self, redis_url: str) -> None:
        # redis 7.x has type stubs but mypy in CI doesn't recognize them
        self._redis = redis.from_url(redis_url)  # type: ignore[no-untyped-call,no-untyped-def]
